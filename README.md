...
```tsx
import { Genui, GenUISurface, useGenui } from '@agentskillmania/genui';

function MyApp() {
  const genui = useGenui();
  const { surfaceManager } = genui;
...
```tsx
import { registerComponent, getComponentRenderer } from '@agentskillmania/genui';
