...
describe('Chart component', () => {
  ...
  describe('negative paths', () => {
    it('should handle missing config gracefully', () => {
      render(<Chart id="c" component="Chart" properties={{ chartType: 'bar', data: [{ a: 1 }] }} />);
    ...
    it('should handle undefined chartType by defaulting to bar', () => {
      render(<Chart id="c" component="Chart" properties={{ data: [{ a: 1 }], config: {} }} />);
    ...
    it('should handle undefined data by defaulting to empty', () => {
      render(<Chart id="c" component="Chart" properties={{ chartType: 'bar', config: {} }} />);
    ...
    it('should handle null data (falls back via ||)', () => {
      render(<Chart id="c" component="Chart" properties={{ chartType: 'bar', data: null as unknown as unknown[] }} />);
    ...
    it('should handle null config (falls back via ||)', () => {
      render(<Chart id="c" component="Chart" properties={{ chartType: 'bar', data: [], config: null as unknown as Record<string, unknown> }} />);
...
describe('buildEChartsOption', () => {
  ...
  describe('negative paths', () => {
    it('should handle empty data array', () => {
      const option = buildEChartsOption('bar', [], { xField: 'name', yField: 'value' });
      expect(option.series[0].data).toEqual([]);
    ...
    it('should return base option for unknown chart type', () => {
      ...
      const option = buildEChartsOption('unknown' as 'bar', sampleData, { xField: 'category', yField: 'amount' });
    ...
    it('should handle pie chart without angleField', () => {
      const option = buildEChartsOption('pie', sampleData, { colorField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });
    ...
    it('should handle scatter chart without field keys', () => {
      ...
      expect(option.series[0].data).toEqual([[0, 0], [0, 0]]);
    });
    ...
    it('should handle gauge chart with gaugeLabel', () => {
      const option = buildEChartsOption('gauge', sampleData, { yField: 'amount', gaugeLabel: 'Speed' });
    ...
    it('should omit legend when legendVisible is false', () => {
      const option = buildEChartsOption('bar', sampleData, { xField: 'category', yField: 'amount', legendVisible: false });
      expect(option.legend).toBeUndefined();
    });

    it('should handle pie chart without colorField (name defaults to empty)', () => {
      const option = buildEChartsOption('pie', sampleData, { angleField: 'amount' });
      expect(option.series[0].data).toEqual([
        { name: '', value: 5 },
        { name: '', value: 15 },
      ]);
    ...
    it('should handle donut chart without angleField', () => {
      const option = buildEChartsOption('donut', sampleData, { colorField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('should handle donut chart without colorField', () => {
      const option = buildEChartsOption('donut', sampleData, { angleField: 'amount' });
    ...
    it('should handle funnel chart without colorField', () => {
      const option = buildEChartsOption('funnel', sampleData, { angleField: 'amount' });
      expect(option.series[0].data[0].name).toBe('');
    });
    ...
    it('should handle treemap chart without colorField', () => {
      const option = buildEChartsOption('treemap', sampleData, { angleField: 'amount' });
      expect(option.series[0].data[0].name).toBe('');
    });

    it('should handle heatmap chart without xField', () => {
      const option = buildEChartsOption('heatmap', sampleData, { yField: 'amount', colorField: 'category' });
      // Without xKey, categories is empty, so xAxis data is []
      expect((option.xAxis as Record<string, unknown>).data).toEqual([]);
    });

    it('should handle heatmap chart without colorField', () => {
      const option = buildEChartsOption('heatmap', sampleData, { xField: 'category', yField: 'amount' });
      expect((option.yAxis as Record<string, unknown>).data).toEqual([]);
    });

    it('should handle heatmap chart without yField', () => {
      const option = buildEChartsOption('heatmap', sampleData, { xField: 'category', colorField: 'category' });
    ...
    it('should handle bar_grouped without colorField (falls back to single series)', () => {
      const option = buildEChartsOption('bar_grouped', sampleData, { xField: 'category', yField: 'amount' });
    ...
    it('should handle bar_grouped without yField (falls back to single series)', () => {
      const option = buildEChartsOption('bar_grouped', sampleData, { xField: 'category', colorField: 'category' });
    ...
    it('should handle line chart without yField', () => {
      const option = buildEChartsOption('line', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('should handle area chart without yField', () => {
      const option = buildEChartsOption('area', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('should handle column chart without yField', () => {
      const option = buildEChartsOption('column', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('should handle radar chart without yField', () => {
      const option = buildEChartsOption('radar', sampleData, { xField: 'category' });
      expect(option.series[0].data[0].value).toEqual([]);
    });

    it('should handle wordCloud without yField', () => {
      const option = buildEChartsOption('wordCloud', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('should handle scatter chart data values that are null/undefined', () => {
      const dataWithNulls = [{ x: null, y: undefined }];
      const option = buildEChartsOption('scatter', dataWithNulls, { xField: 'x', yField: 'y' });
    ...
    it('should handle pie chart with null colorField values (?? branch)', () => {
      ...
      const option = buildEChartsOption('pie', dataWithNullColor, { angleField: 'value', colorField: 'name' });
    ...
    it('should handle donut chart with null colorField values', () => {
      ...
      const option = buildEChartsOption('donut', dataWithNullColor, { angleField: 'value', colorField: 'name' });
      expect(option.series[0].data[0].name).toBe('');
    });

    it('should handle funnel chart with null colorField values', () => {
      const dataWithNullColor = [{ name: null, value: 10 }];
      const option = buildEChartsOption('funnel', dataWithNullColor, { angleField: 'value', colorField: 'name' });
      expect(option.series[0].data[0].name).toBe('');
    });

    it('should handle treemap chart with null colorField values', () => {
      const dataWithNullColor = [{ name: null, value: 10 }];
      const option = buildEChartsOption('treemap', dataWithNullColor, { angleField: 'value', colorField: 'name' });
      expect(option.series[0].data[0].name).toBe('');
    });

    it('should handle rose chart with null colorField values', () => {
      const dataWithNullColor = [{ name: null, value: 10 }];
      const option = buildEChartsOption('rose', dataWithNullColor, { angleField: 'value', colorField: 'name' });
      expect(option.series[0].data[0].name).toBe('');
    });

    it('should handle rose chart with angleField but without colorField', () => {
      const option = buildEChartsOption('rose', sampleData, { angleField: 'amount' });
    ...
    it('should handle heatmap chart with null xField values', () => {
      ...
      const option = buildEChartsOption('heatmap', dataWithNulls, { xField: 'cat', yField: 'val', colorField: 'color' });
    ...
    it('should handle buildBarSeries grouped with null colorField values', () => {
      const dataWithNulls = [
        { month: 'Jan', product: null, sales: 10 },
      ];
    ...
    it('should handle scatter chart with null xField values in categories', () => {
      ...
      const option = buildEChartsOption('bar', dataWithNull, { xField: 'cat', yField: 'val' });
      // d[cat] ?? '' → ''
      expect((option.xAxis as Record<string, unknown>).data).toEqual(['']);
    });
  });
  ...
  describe('sankey chart', () => {
    it('maps sankey chart with nodes and links', () => {
      ...
      expect(option.series[0].links).toEqual([{ source: 'a', target: 'b', value: 10 }]);
    });
  ...
  describe('boxplot chart', () => {
    it('maps boxplot chart with category axis', () => {
      ...
      const option = buildEChartsOption('boxplot', boxData, { xField: 'name', yField: 'values' });
    ...
    it('handles boxplot without yField', () => {
      const option = buildEChartsOption('boxplot', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });
  });
  ...
  describe('candlestick chart', () => {
    it('maps candlestick chart with category axis', () => {
      ...
      const option = buildEChartsOption('candlestick', stockData, { xField: 'date', yField: 'ohlc' });
    ...
    it('handles candlestick without yField', () => {
      const option = buildEChartsOption('candlestick', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });
  });
  ...
  describe('themeRiver chart', () => {
    it('maps themeRiver chart with x, y, color fields', () => {
      const option = buildEChartsOption('themeRiver', sampleData, {
        xField: 'category', yField: 'amount', colorField: 'category',
      });
  ...
  describe('graph chart', () => {
    ...
    it('respects custom layout config', () => {
      const option = buildEChartsOption('graph', [], { nodes: [], links: [], layout: 'circular' });
  ...
  describe('parallel chart', () => {
    it('maps parallel chart with dimensions', () => {
      ...
      const option = buildEChartsOption('parallel', sampleData, { dimensions: dims });
  ...
  describe('pictorialBar chart', () => {
    ...
    it('handles pictorialBar without yField', () => {
      const option = buildEChartsOption('pictorialBar', sampleData, { xField: 'category' });
      expect(option.series[0].data).toEqual([]);
    });

    it('uses custom symbolSize when provided', () => {
      const option = buildEChartsOption('pictorialBar', sampleData, {
        xField: 'category', yField: 'amount', symbolSize: [30, 30],
      });
