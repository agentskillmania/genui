...
const mockEngine = {
  ...
  getSurface: vi.fn<(id: string) => typeof mockSurfaceState | undefined>(
    () => mockSurfaceState,
  ),
  resolveProperties: vi.fn<(surfaceId: string, value: unknown) => unknown>(
    (_sid, value) => value,
  ),
  addListener: vi.fn<(listener: (event: SurfaceEvent) => void) => () => void>(
    (listener) => {
...
describe("GenUISurface", () => {
  ...
  it("submits UI action via surfaceManager on component action", () => {
    mockGetRenderer.mockReturnValue(((props: {
      id: string;
